...
export function renderHelp(): string {
  ...
  return [
    ...
    "",
    `  ${goldSoft("mneme beam · live schema feed")}  ${dim("(tail your agent's brain)")}`,
    tip("/beam",         "stream every INSERT / UPDATE / DELETE in real-time"),
    "",
    `  ${goldSoft("mneme mesh · agent memory marketplace")}  ${dim("(sell + buy agent memory)")}`,
    tip("/mesh discover","browse listings across all projects"),
    tip("/mesh list",    "publish one of your tables for paid querying"),
    tip("/mesh query id","buy + run a query against any listing"),
    tip("/mesh credits", "your buyer-side USDC credits"),
    tip("/mesh sales",   "your seller-side dashboard"),
    "",
    `  ${goldSoft("mneme mandate · declarative agent intents")}  ${dim("(metamask-ready)")}`,
    tip("/mandate create",  "publish a swap / send / lp / perp intent"),
    tip("/mandates",        "list mandates (filter by status=…)"),
    tip("/mandate arm",     "start watching conditions"),
    tip("/mandate cancel",  "cancel a pending or armed mandate"),
    "",
    `  ${goldSoft("mneme chronos · time-travel + proofs")}  ${dim("(your agent cannot rewrite its past)")}`,
    tip("/rewind [t] [at]", `table state AS OF a time: ${dim("/rewind memories 7d")}`),
    tip("/diff [t] [from]", "what changed in a window"),
    tip("/journal [N]",     "raw write journal, newest first"),
    tip("/anchor",          "Merkle-root the journal (posts to Base if configured)"),
    tip("/anchors",         "list anchors + onchain tx hashes"),
    tip("/prove <id>",      "Merkle proof: journal entry → anchored root"),
    "",
    `  ${goldSoft("session")}`,
