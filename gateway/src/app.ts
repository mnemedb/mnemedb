/**
 * Mneme gateway — Hono app (routes + middleware only).
 *
 * No server, no workers, no DB init here — so the same app can be served
 * by Bun locally (src/index.ts) or by a Vercel serverless function
 * (api/index.ts at the repo root).
 */
